    ...
    <React.Fragment>
      <div className="container">
        ...
        <button style={toggleBtnStyle} onClick={() => setCollapsed(!collapsed)}>
          {collapsed ? <i className="bi bi-arrow-right-circle-fill collapse-icon"></i> : <i className="bi bi-arrow-left-circle-fill collapse-icon"></i>}
        </button>
        <Nav style={{display:"grid", rowGap:"5px", alignItems: "center"}}>
          <Nav.Link href="#home" className="flex-center">
            <i className="bi bi-plus-circle-fill new-chat-icon"></i>
            <span className='menu-text-new-chat'>{!collapsed && "New Chat"}</span>
          </Nav.Link>
          <Nav.Link href="#about" className="flex-center">
            <i className="bi bi-wechat menu-icons"></i>
            <span className='menu-text'>{!collapsed && "Chats"}</span>
          </Nav.Link>
          <Nav.Link href="#services" className="flex-center">
            <i className="bi bi-journal-text menu-icons"></i>
            <span className='menu-text'>{!collapsed && "Library"}</span>
          </Nav.Link>
        </Nav>
        {!collapsed && 
        <div style={{display: "flex", flexDirection: "column", height: "100%", maxHeight: "59%"}}>
        <hr style={{ border: "1px solid gray", margin: "80px 0 8px 0" }} />
        <div className="recent-chats-section" style={{flexGrow:1, overflowY: "auto"}}>
          <Nav style={{display:"grid", rowGap:"5px", alignItems: "center"}}>
            <Nav.Link className="flex-center">
              <span className='recent-chat'>{!collapsed && "Recent Chats"}</span>
            </Nav.Link>
            <Nav.Link href="#about" className="flex-center flex-inlin" >
              <i className="bi bi-stars menu-icon m-r-10"></i>
              <span className='menu-text two-line-ellipsis'>{!collapsed && "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."}</span>
            </Nav.Link>
            <Nav.Link href="#about" className="flex-center flex-inlin" >
              <i className="bi bi-stars menu-icon m-r-10"></i>
              <span className='menu-text two-line-ellipsis'>{!collapsed && "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat."}</span>
          ...
          <div className='profile-icon-collapsed'>
            ...
            >
              <Popover id="popover-contained">
                <div style={{display:"flex", flexDirection:"column", rowGap:"10px", padding:"10px"}}>
